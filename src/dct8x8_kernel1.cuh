...
__global__ void CUDAkernel1DCT(float *Dst, int ImgWidth, int OffsetXBlocks, int OffsetYBlocks)
{
	...
	if(tex_x==STOPATXPOSTION && tex_y==STOPATYPOSTION){
		printf("data in kernel:x=%f,y=%f, value=%f \n",
				tex_x,tex_y, tex2D(TexSrc, tex_x, tex_y));
	}
...
#pragma unroll
	...
	int imgIndex1= FMUL(((by << BLOCK_SIZE_LOG2) + ty), ImgWidth);
	int imgIndex2=((bx << BLOCK_SIZE_LOG2) + tx);
	int blockindex=(ty << BLOCK_SIZE_LOG2) + tx;
	Dst[ imgIndex1 + imgIndex2 ] = CurBlockLocal1[blockindex ];

	//printf("dct data in kernel:x=%d,y=%d, value=%f \n", imgIndex2,imgIndex1/ImgWidth,CurBlockLocal1[blockindex]);
}
