...
void writeToTextFile(char *pfilename,float *ImgDctF, int ImgDctFStride,int width,int height){
	...
	for(int h=0; h< height; h++){
		for(int w=0; w<width; w++){
			int index1 = h*ImgDctFStride + w;
			char buff[300];
			sprintf(buff, "x=%d,y=%d, value = %f",w,h,ImgDctF[index1]);
			std::string buffAsStdStr = buff;
			textfile << buffAsStdStr << endl;
			if(ImgDctF[index1] < min){
				min = ImgDctF[index1];
			}
			if(ImgDctF[index1] > max){
				max = ImgDctF[index1];
			}
		}
	}
	textfile.close();
	printf("file: %s, min=%f,max=%f\n",pfilename,min, max);
}
bool compareDctresult(string fname1, string fname2){
	bool result = true;
	...
	while (getline(file1, str1))
	{
		// Process str
		getline(file2, str2);
		if(getvalueatxyposition(str1)!=getvalueatxyposition(str2)){
			printf("Error: difference %s in file %s, %s in file %s\n",
					str1.c_str(),fname1.c_str(),str2.c_str(),fname2.c_str());
			result = false;
			i++;
		}

	}
	printf("difference number:%d, %s, %s\n", i, fname1.c_str(),fname2.c_str());
	return result;
}
void transformstdDCTresult(float *ImgDctF, int ImgDctFStride,int width,int height){
	for(int y=0; y< height; y++){
		for(int x=0; x<width; x++){
			int index1 = y*ImgDctFStride + x;
			byte val = (byte)((abs(ImgDctF[index1]))*0.1f);
			if(val>255){
				val = 255;
			}
			ImgDctF[index1]=val;
...
float WrapperCUDA1(byte *ImgSrc, byte *ImgDst, float *ImgDctF,int ImgDctFStride,int Stride, ROI Size)
{
	...
	writeToTextFile("std_originalvalues_1.txt",ImgSrcF, ImgSrcFStride,Size.width,Size.height);
	AddFloatPlane(-128.0f, ImgSrcF, ImgSrcFStride, Size);
	writeToTextFile("std_originalvalues_2.txt",ImgSrcF, ImgSrcFStride,Size.width,Size.height);
	...
	//copy out the dct values

	checkCudaErrors(cudaMemcpy2D(ImgDctF, ImgDctFStride *sizeof(float),
			Dst, DstStride *sizeof(float),
			Size.width *sizeof(float), Size.height,
			cudaMemcpyDeviceToHost));
	writeToTextFile("std_dctvalues.txt",ImgDctF, ImgDctFStride,Size.width,Size.height);
	transformstdDCTresult(ImgDctF, ImgDctFStride, Size.width, Size.height);
	writeToTextFile("std_dctvalues_transformed.txt",ImgDctF, ImgDctFStride,Size.width,Size.height);
...
int dcttestmain(int argc, char **argv)
{
	...
	char SampleImageFname[] = "sample_0.png";
	...
	ROI ImgSize;
	int res = PreLoadBmp2(pSampleImageFpath, &ImgWidth, &ImgHeight);
	...
	int ImgDctFStride;
	float *ImgDctF = MallocPlaneFloat(ImgWidth, ImgHeight, &ImgDctFStride);

	//load sample image
	// LoadBmpAsGray(pSampleImageFpath, ImgStride, ImgSize, ImgSrc);
	LoadBmpAsGray4("./data/sample_0.png.txt", ImgStride, ImgSize, ImgSrc);
	...
	float TimeCUDA1 = WrapperCUDA1(ImgSrc, ImgDstCUDA1, ImgDctF,ImgDctFStride,ImgStride, ImgSize);
	...
	printf("Processing time (Gold 1)    : %f ms \n", TimeGold1);
	printf("Processing time (Gold 2)    : %f ms \n", TimeGold2);
	...
	//write doppia result to file
	int ImgDctFStrideDoppia;
	float *ImgDctFDoppia = MallocPlaneFloat(ImgWidth, ImgHeight, &ImgDctFStrideDoppia);
	//	LoadBmpAsGray3("./data/sample_0_dct.png", ImgDctFStrideDoppia, ImgSize, ImgDctFDoppia);
	//	writeToTextFile("doppia_dctvalues.txt",ImgDctFDoppia, ImgDctFStrideDoppia,ImgWidth,ImgHeight);


	//ensure original input file is the same between doppia and cuda sample
	bool orignPassed=compareDctresult("./data/sample_0.png.txt","./std_originalvalues_1.txt");
	//Compare dct results
	bool dctPassed=compareDctresult("./data/sample_0.png_dct.txt","./std_dctvalues_transformed.txt");
	...
	bTestResult = dctPassed && bTestResult && orignPassed;
	//
	// Finalization
	//
